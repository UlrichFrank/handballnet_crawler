#!/usr/bin/env python3
"""
Simplified extraction that uses pre-authenticated cookies instead of re-authenticating
"""
...
try:
    ...
    for idx, game_id in enumerate(game_ids[:5], 1):  # Test with first 5 games
        try:
            ...
            print(f"  [{idx}/{len(game_ids[:5])}] {game_id}: ", end="")
            ...
            for team_name, players in players_by_team.items():
                ...
                print(f"{team_name} ({len(player_names)})", end=" | ")
            
            print()
            game_results.append(game_data)
            
        except Exception as e:
            print(f"  [{idx}/{len(game_ids[:5])}] ❌ Error: {str(e)[:50]}")
    ...
    # Export to JSON
    output_file = 'extracted_players_sample.json'
