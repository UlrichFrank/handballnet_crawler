...
export function TeamDefenseTable({ league }: TeamDefenseTableProps) {
  const [teams, setTeams] = useState<TeamDefenseStats[]>([]);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true);
        const stats = await dataService.getTeamDefenseStats(league.name);
        setTeams(stats);
      } catch (err) {
        setError(err instanceof Error ? err.message : 'Defensivstatistiken konnten nicht geladen werden');
      } finally {
        setLoading(false);
      }
    };
          ...
          <tbody>
            {teams.map((team, idx) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-center text-gray-900 dark:text-gray-100 font-bold">{team.totalConceded}</td>
                <td className="px-4 py-3 text-center text-gray-900 dark:text-gray-100">{team.games}</td>
                <td className="px-4 py-3 text-center text-gray-900 dark:text-gray-100 font-semibold">{team.avgConcededPerGame}</td>
              </tr>
            ))}
          </tbody>
        </table>
